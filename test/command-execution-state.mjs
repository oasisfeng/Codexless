...

{
  ...
  const state = createCommandExecutionState({ executor, maxConcurrent: 2, terminalTtlMs: 1_000, maxTerminalCommands: 10 });

  const synchronous = state.exec({ command: ["sync"], access: "readOnly", timeoutMs: 100 });
  const started = state.start({ command: ["async"], access: "inherit", timeoutMs: 200 });
  assert.match(started.commandRef, /^cmd_[0-9a-f-]{36}$/);
  assert.equal(state.poll(started.commandRef).status, "running");
  assert.throws(
  ...
  assert.deepEqual(completed.input.command, ["async"]);
...

{
  ...
  const state = createCommandExecutionState({ executor, maxConcurrent: 1, terminalTtlMs: 1_000, maxTerminalCommands: 10 });
...

{
  ...
  assert.throws(() => state.poll(started.commandRef), /unknown or expired commandRef/);
}
...

{
  ...
  const state = createCommandExecutionState({ executor, maxConcurrent: 1, terminalTtlMs: 1_000, maxTerminalCommands: 1 });
  ...
  await delay(0);
  assert.throws(() => state.poll(first.commandRef), /unknown or expired commandRef/, "terminal count cap must retire the oldest terminal job");
  assert.equal(state.poll(second.commandRef).status, "completed");
  await state.drain();
}

{
  const executor = { exec: async () => ({ exitCode: 0, stdout: "TTL", stderr: "" }) };
  const state = createCommandExecutionState({ executor, maxConcurrent: 1, terminalTtlMs: 20, maxTerminalCommands: 10 });
  const started = state.start({ command: ["ttl"], access: "readOnly", timeoutMs: 100 });
  await delay(0);

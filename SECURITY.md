...
- `codex.command_start` uses the same authority-bounded `command/exec` path as `codex.command_exec`; it does not use the broader host `process/spawn` lane. Its `commandRef` is an unguessable in-memory bearer capability: possession is sufficient to poll that result, refs are not enumerable, and terminal results have no wall-clock expiry. They remain readable for the current runtime until displaced by bounded terminal-result retention or runtime restart; terminal records retain only the minimal result-projection metadata rather than the original argv/timeout.
